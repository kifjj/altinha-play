...
- the second increment was more challenging. It uses the Yolo Pose to detect the key body parts points and then calculates the closest kbpp to the ball in the detected hit frame.
    - Detecting the closest person is not accurate, very dependent on the video angle, person distance (players are not detected), needs more work. Perhaps (use the ball size to detect the closes person or tracking logic)
